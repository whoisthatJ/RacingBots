...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        [SerializeField] private bool isOvertaking = false;                // Whether the car is overtaking
        private bool isChangingLanes = false;
        public float raycastDistance = 20f;               // Distance to detect cars ahead
        public float overtakeSpeedMultiplier = 1.5f;      // Speed multiplier when overtaking
        public float laneChangeDistance = 2f;             // Distance to change lanes
        ...
        private void FixedUpdate()
        {
            ...
            else
            {
                ...
                if (!isOvertaking) {
                    ...
                    steer = m_CarController.MaxSpeed;
                }

                if (backingOff) {
                    accel = -m_CarController.MaxSpeed;
                    steer = 0;
                }
        ...
        private void DetectCarInFront() {
            RaycastHit hit;
            Debug.DrawRay(transform.position + Vector3.up * 0.5f + transform.forward * 5, transform.forward * raycastDistance, Color.red);
            // Raycast forward to detect if there's a car ahead
            if (Physics.Raycast(transform.position + Vector3.up * 0.5f, transform.forward * 5, out hit, raycastDistance)) {
                Debug.Log(m_Rigidbody.velocity.magnitude + " " + hit.collider.gameObject.name);
                if (hit.collider.CompareTag("Car") && m_Rigidbody.velocity.magnitude > hit.rigidbody.velocity.magnitude) {
                    Debug.Log("Car detected in front, initiating overtake.");
        ...
        IEnumerator Overtake() {
            ...
            // Check if the adjacent lane is clear before lane change
            if (CheckLaneClear(Vector3.right)) {
                yield return StartCoroutine(ChangeLane(0.3f));
            }
            else if (CheckLaneClear(Vector3.left)) {
                yield return StartCoroutine(ChangeLane(-0.3f));
            }

            // Increase speed during overtaking
            float originalSpeed = m_CarController.CurrentSpeed;
            m_CarController.Move(0, overtakeSpeedMultiplier, overtakeSpeedMultiplier, 0f);

            // Wait for a few seconds to simulate overtaking duration
            yield return new WaitForSeconds(0.5f);

            // Change back to the original lane
            if (isChangingLanes) {
                yield return StartCoroutine(ChangeLane(0));
            }

            // Restore original speed
            m_CarController.Move(0, originalSpeed, originalSpeed, 0f);

            isOvertaking = false;
        }

        private bool CheckLaneClear(Vector3 direction) {
            // Cast a ray in the direction of the adjacent lane to check if it's clear
            return !Physics.Raycast(transform.position + Vector3.up * 0.5f, transform.TransformDirection(direction), laneChangeDistance);
        }
        IEnumerator ChangeLane(float direction) {
            isChangingLanes = true;
            float laneChangeTime = 0.2f;  // Time taken to change lanes
            float elapsedTime = 0;

            while (elapsedTime < laneChangeTime) {
                m_CarController.Move(direction, m_CarController.MaxSpeed, m_CarController.MaxSpeed, 0f);
